...
class TutorCard extends Component {
  ...
  renderStar(id) {
    const isChosen =
      this.state.tempRate > 0
        ? id <= this.state.tempRate
        : id <= this.state.rate;
    return (
      <i
        id={`star-${id}`}
        className={`fa ${isChosen ? "fa-star" : "fa-star-o"} mr-3`}
        style={{
          "font-size": 30,
          cursor: "pointer",
          color: isChosen ? "#b74d33" : "gray"
        }}
    ...
    return (
      <div
        ...
        style={{
          ...
          width: 300,
          backgroundColor: "#FFF",
          "box-shadow": "5px 4.5px 10px 0 rgba(0, 0, 0, 0.16)"
        }}
      >
        <div className="row p-0 m-0" style={{ height: 250, width: "100%" }}>
          <img
            className=""
            src={this.props.img || "/img/tutor/tutor-2.png"}
            style={{
              height: "100%",
              width: "100%",
              cursor: "pointer",
        ...
        <div
          className="border-bottom border-secondary row m-0 p-0"
          style={{ height: 200, width: "100%" }}
        >
          <div
            className="row text m-0 p-3"
            style={{ "font-size": 20, color: "#615959", cursor: "default" }}
          >
            {this.props.name || "Grand Marshall"}
          </div>
          <div
            className="row text text-muted m-0 pt-0 pb-1 pl-3 pr-3"
            style={{ "font-size": 15 }}
          >
            <p
              style={{
